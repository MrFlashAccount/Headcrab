...
const TOKENIZED_BEGIN_DELIMITER_PATTERN =
  /<<<BEGIN_ORIGINAL_TASK:[a-f0-9-]+\.[a-f0-9]{16}>>>/g;
const TOKENIZED_END_DELIMITER_PATTERN =
  /<<<END_ORIGINAL_TASK:[a-f0-9-]+\.[a-f0-9]{16}>>>/g;
...
function isBoundedTelemetryPayload(payload) {
  ...
  if (
    Object.hasOwn(payload, "variant") &&
    !ALLOWED_VARIANTS.has(payload.variant)
  ) {
    return false;
  }
...
export function runDelegateModeHealthcheck() {
  ...
  addCheck(
    ...
    typeof promptResult?.prependContext === "string" &&
      countOccurrences(promptBlock, DELEGATE_REMINDER_MARKER) === 1
  );
  addCheck(
    "promptReminder.bounded_size",
    byteLength(promptBlock) > 0 &&
      byteLength(promptBlock) <= MAX_PREPEND_CONTEXT_BYTES
  );

  const repeatedPromptResults = Array.from({ length: 3 }, () =>
    adapter.beforePromptBuild(
      { prompt: "synthetic prompt after compaction", messages: [] },
      HEALTHCHECK_SESSION_CONTEXT
    )
  );
  addCheck(
  ...
  const firstWrap = adapter.beforeToolCall(
    {
      ...
      params: { task: "synthetic delegated task", label: "healthcheck" },
    },
    HEALTHCHECK_SESSION_CONTEXT
  );
  ...
  const secondWrap = firstWrap
    ? adapter.beforeToolCall(
        { toolName: "sessions_spawn", params: firstWrap.params },
        HEALTHCHECK_SESSION_CONTEXT
      )
    : undefined;
  ...
  addCheck(
    ...
    typeof wrappedTask === "string" &&
      wrappedTask.startsWith(FORTHRIGHT_WORKER_PREFIX)
  );
  addCheck(
  ...
  addCheck(
    ...
    calls.length > 0 &&
      ...
      calls.every(
        ({ payload }, index) =>
          isBoundedTelemetryPayload(payload) && payload.n === index + 1
      )
  );
